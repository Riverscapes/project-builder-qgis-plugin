...
class RCAproject:
    ...
    def __init__(self, projectName, projPath, ex_veg, hist_veg, network, frag_valley, rca, lrp="", thiessen="",
        ...
        newxml.addRCARealization("RCA Realization 1", rid="RZ1", dateCreated=self.time.strftime("%Y-%m-%d %H:%M:%S"),
                                 guid=self.getUUID())
    ...
    def copy_datasets(self, projPath, ex_veg, hist_veg, network, frag_valley, rca, lrp, thiessen, newxml):
        ...
        exvegpath = "01_Inputs/01_Ex_Cov/Ex_Cov_1/" + os.path.basename(ex_veg)
        shutil.copytree(ex_veg, exvegpath)

        newxml.addProjectInput("Raster", "Existing Cover", str(exvegpath), iid="EXVEG1", guid=self.getUUID())
        newxml.addRCAInput(newxml.RCArealizations[0], "Existing Cover", ref="EXVEG1")

        histvegpath = "01_Inputs/02_Hist_Veg/Hist_Cov_1/" + os.path.basename(hist_veg)
        shutil.copytree(hist_veg, histvegpath)

        newxml.addProjectInput("Raster", "Historic Cover", str(histvegpath), iid="HISTVEG1", guid=self.getUUID())
        newxml.addRCAInput(newxml.RCArealizations[0], "Historic Cover", ref="HISTVEG1")
        ...
        frag_valley_copy = "01_Inputs/04_Frag_Valley/Frag_Valley_1/" + os.path.basename(frag_valley)
        ...
        rca_copy = "02_Analyses/Output_1/" + os.path.basename(rca)
        inRca = ogr.GetDriverByName("ESRI Shapefile").Open(rca)
        ogr.GetDriverByName("ESRI Shapefile").CopyDataSource(inRca, rca_copy)

        newxml.addOutput("Analysis 1", "Vector", "RCA 1", rca_copy, newxml.RCArealizations[0], guid=self.getUUID())
